...
public class StaticQueue<E> implements Queue<E> {
	// êndice para o primeiro elemento
	protected int first;

	// êndice para o œltimo elemento
	protected int last;
	...
	//fila > pilha onde o primeiro da fila deverá ser o topo da pilha
	//pilha > lista onde o topo da pilha deverá ficar na última posição da lista
	//lista > fila onde o primeiro elemento da lista deve ser o último da fila

	public boolean trocaElementos(StaticStack<E> stack, StaticList<E> list){
		if(stack.numElements() != list.numElements() || list.numElements() != numElements())
			return false;
		try{	
			StaticStack<E> stackN = new StaticStack<>(stack.numElements());
			StaticList<E> listN = new StaticList<>(list.numElements());	
			...
			for (int i = stack.numElements(); i > stack.numElements(); i--) {
				listN.insert(stack.pop(), i);
			}
			...
			for (int i = list.numElements(); i > list.numElements(); i--) {
				enqueue(list.get(i));
			}
